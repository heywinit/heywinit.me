...
export default function LocationCard() {
  ...
  useEffect(() => {
    ...
    const globe = createGlobe(canvasRef.current, {
      ...
      markers: [{ location: [23.0225, 72.5714], size: 0.1 }], // Ahmedabad coordinates
      scale: 1.05,
      onRender: (state) => {
        state.phi = 3.315;
        state.width = width * 2;
  ...
  return (
    <div className="relative flex h-60 flex-col gap-6 overflow-hidden rounded-lg border border-border bg-card p-6">
      <div className="flex items-center gap-2">
        <MapPin className="h-[18px] w-[18px] text-primary" />
        <h2 className="text-sm font-light">Ahmedabad</h2>
      </div>
